...
export const addEnrolledScheme = async (req, res) => {
  try {
    ...
    // Add new scheme
    const newScheme = {
      _id: new mongoose.Types.ObjectId(), // Explicitly generate ID
      schemeName,
      schemeId,
    ...
    // Use findByIdAndUpdate to push and return the updated document
    // This is more reliable than updateOne for getting the result back
    const updatedUser = await User.findByIdAndUpdate(
      userId,
      { $push: { enrolledSchemes: newScheme } },
      { new: true, runValidators: false } // runValidators: false to avoid full document validation issues if any
    );

    if (!updatedUser) {
      throw new Error('Failed to update user with new scheme');
    }
